...
def test_allocate_2_gpus_auto(device_manager, mock_callback):
    ...
    devices = device_manager.request_device("tool1", "instance1", device="cudax2", eviction_callback=mock_callback)
...
def test_multi_gpu_lru_eviction(device_manager, mock_callback):
    ...
    device_manager.request_device("tool3", "instance3", device="cuda", eviction_callback=mock_callback)

    assert device_manager._allocations["instance1"].device_ids == ["cpu"], "instance1 should be evicted to CPU"
    assert device_manager._allocations["instance2"].device_ids == ["cuda:0"], "instance2 should be on cuda:0"
    assert device_manager._allocations["instance3"].device_ids == ["cuda:1"], "instance3 should be on cuda:1"
...
def test_multi_gpu_request_evicts_two_single_instances(device_manager, mock_callback):
    ...
    assert device_manager._allocations["instance1"].device_ids == ["cuda:0"], "instance1 should be on cuda:0"
    assert device_manager._allocations["instance2"].device_ids == ["cuda:1"], "instance2 should be on cuda:1"

    devices = device_manager.request_device("tool3", "instance3", device="cudax2", eviction_callback=mock_callback)

    assert device_manager._allocations["instance1"].device_ids == ["cpu"], "instance1 should be evicted to CPU"
    assert device_manager._allocations["instance2"].device_ids == ["cpu"], "instance2 should be evicted to CPU"
...
def test_lru_eviction_multi_gpu_request():
    ...
    with patch(
        ...
        dm.request_device("tool1", "instance1", device="cuda:0", eviction_callback=cb1)
        time.sleep(0.01)
        dm.request_device("tool2", "instance2", device="cuda:1", eviction_callback=cb2)
        time.sleep(0.01)
        dm.request_device("tool3", "instance3", device="cuda:2", eviction_callback=cb3)
        ...
        dm.request_device("tool4", "instance4", device="cudax2", eviction_callback=cb4)

        assert dm._allocations["instance1"].device_ids == ["cpu"], "instance1 should be evicted to CPU"
        assert dm._allocations["instance2"].device_ids == ["cpu"], "instance2 should be evicted to CPU"
        assert dm._allocations["instance3"].device_ids == ["cuda:2"], "instance3 should still be on cuda:2"
...
def test_multi_gpu_move():
    ...
    with patch(
        ...
        dm.request_device("tool1", "instance1", device="cuda:0,1", eviction_callback=cb)
...
def test_multi_gpu_release(device_manager, mock_callback):
    ...
    devices = device_manager.request_device("tool2", "instance2", device="cudax2", eviction_callback=mock_callback)
    assert devices == "cuda:0,cuda:1"
...
def test_mixed_single_and_multi_gpu():
    ...
    with patch(
        ...
        result1 = dm.request_device("tool1", "instance1", device="cuda", eviction_callback=cb)
        assert result1 == "cuda:0"

        result2 = dm.request_device("tool2", "instance2", device="cudax2", eviction_callback=cb)
