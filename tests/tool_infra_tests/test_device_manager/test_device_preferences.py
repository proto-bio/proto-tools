...
def test_cuda_allocation_stays_on_general_cuda_request(device_manager, mock_callback):
    ...
    device = device_manager.request_device("tool1", "inst1", device="cuda:0", eviction_callback=mock_callback)
    assert device == "cuda:0"

    device2 = device_manager.request_device("tool1", "inst1", device="cuda", eviction_callback=mock_callback)
...
def test_cuda_allocation_stays_on_same_specific_request(device_manager, mock_callback):
    ...
    device2 = device_manager.request_device("tool1", "inst1", device="cuda:0", eviction_callback=mock_callback)
...
def test_cpu_allocation_stays_on_cpu_request(device_manager, mock_callback):
    ...
    device_manager.request_device("tool1", "inst1", device="cpu", eviction_callback=mock_callback)
    device2 = device_manager.request_device("tool1", "inst1", device="cpu", eviction_callback=mock_callback)
...
def test_cuda_allocation_moves_on_different_specific_request(device_manager, mock_callback):
    ...
    device_manager.request_device("tool1", "inst1", device="cuda:0", eviction_callback=mock_callback)
    device2 = device_manager.request_device("tool1", "inst1", device="cuda:1", eviction_callback=mock_callback)
...
def test_single_gpu_allocation_re_allocates_on_multi_gpu_request(device_manager, mock_callback):
    ...
    device_manager.request_device("tool1", "inst1", device="cuda:0", eviction_callback=mock_callback)
    device2 = device_manager.request_device("tool1", "inst1", device="cudax2", eviction_callback=mock_callback)
...
def test_preferred_device_busy_with_warning(device_manager, mock_callback):
    ...
    # Occupy cuda:1
    device_manager.request_device("tool1", "instance1", device="cuda:1", eviction_callback=mock_callback)
...
def test_preferred_device_all_busy_no_warning(device_manager, mock_callback):
    ...
    assert not any("but" in str(call) and "are free" in str(call) for call in mock_warning.call_args_list), (
...
def test_allow_multiple_per_device(device_manager, mock_callback):
    ...
    device1 = device_manager.request_device("tool1", "instance1", device="cuda", eviction_callback=mock_callback)
    device2 = device_manager.request_device("tool2", "instance2", device="cuda", eviction_callback=mock_callback)
...
def test_routes_to_least_allocated_gpu(device_manager, mock_callback):
    ...
    device4 = device_manager.request_device("tool4", "inst4", device="cuda", eviction_callback=mock_callback)

    assert device4 == "cuda:1", f"Should route to GPU with fewest allocations (cuda:1), got {device4}"
