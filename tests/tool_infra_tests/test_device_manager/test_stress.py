"""Stress tests for device movement with real tool instances.

Tests use mixtures of PyTorch, JAX, and CLI mock tools to exercise
eviction, movement, restart, and multi-GPU allocation under realistic
conditions.  Each test takes GPU memory snapshots across managed
devices and asserts that memory changes by the expected amount
(~TOOL_MEMORY_MB) on target devices and is unaffected on other
managed devices.
"""
...
# than TOOL_MEMORY_MB so "loaded" vs "freed" states remain distinguishable.
GPU_MEMORY_TOLERANCE_MB = 3072
...
PERSISTENT_GPU_TOOLS = [
    pytest.param(_pytorch_tool, id="pytorch"),
    pytest.param(_jax_tool, id="jax"),
]

MULTI_GPU_PERSISTENT_TOOLS = [
...
def _run_tool(tool_factory, instance_name, **config_kwargs):
    ...
    Defaults ``memory_mb`` to TOOL_MEMORY_MB so all stress tests use a
    consistent, large-enough buffer for reliable memory assertions.
    """
    if "memory_mb" not in config_kwargs:
        config_kwargs["memory_mb"] = TOOL_MEMORY_MB
...
def _assert_gpu_memory(
    ...
    tolerance_mb: float = GPU_MEMORY_TOLERANCE_MB,
    label: str = "",
) -> dict[str, float]:
    ...
    Args:
        ...
        loaded: Devices expected to have ~TOOL_MEMORY_MB above baseline
    ...
    for device, baseline_mb in baseline.items():
        ...
        if device in loaded_set:
            assert abs(delta - TOOL_MEMORY_MB) < tolerance_mb, (
                f"{device}: expected ~{TOOL_MEMORY_MB} MB above baseline{suffix}, "
...
# ============================================================================
# Mixed-Framework Eviction Tests
# ============================================================================


class TestMixedFrameworkEviction:
    """Tests that eviction works correctly across different tool frameworks."""
    ...
    def test_cross_framework_eviction_cpu(self, tool_a_factory, tool_b_factory):
    ...
    def test_cross_framework_eviction_restart(self, tool_a_factory, tool_b_factory):
    ...
    @pytest.mark.uses_gpu
    @pytest.mark.slow
    def test_three_tool_eviction_chain(self):
...
# ============================================================================
# Explicit Device Movement Tests
# ============================================================================


class TestConfigDrivenDeviceMovement:
    """Tests that changing config.device between run() calls moves the tool."""

    @pytest.mark.uses_gpu(2)
    @pytest.mark.slow
    @pytest.mark.parametrize("tool_factory", PERSISTENT_GPU_TOOLS)
    def test_config_moves_between_gpus(self, tool_factory):
    ...
    @pytest.mark.uses_gpu(2)
    @pytest.mark.slow
    @pytest.mark.parametrize("tool_factory", MULTI_GPU_PERSISTENT_TOOLS)
    def test_config_gpu_cpu_gpu_multi_gpu(self, tool_factory):
...
class TestWorkerLifecycle:
    """Tests for worker shutdown, restart, and recovery."""

    @pytest.mark.uses_gpu
    @pytest.mark.slow
    @pytest.mark.parametrize("tool_factory", PERSISTENT_GPU_TOOLS)
    def test_shutdown_and_auto_restart(self, tool_factory):
    ...
    @pytest.mark.uses_gpu
    @pytest.mark.slow
    def test_evict_then_restart_preserves_correctness(self):
...
class TestMultiGPUStress:
    """Tests mixing single-GPU and multi-GPU tools."""

    @pytest.mark.uses_gpu(2)
    @pytest.mark.slow
    def test_multi_gpu_tool_evicts_two_single_gpu_tools(self):
    ...
    @pytest.mark.uses_gpu(2)
    @pytest.mark.slow
    def test_single_gpu_tool_evicts_multi_gpu_tool(self):
    ...
    @pytest.mark.uses_gpu(2)
    @pytest.mark.slow
    def test_rapid_eviction_cycle(self):
...
# ============================================================================
# Multiple Tools Per Device Tests
# ============================================================================


class TestMultipleToolsPerDevice:
    """Tests for allow_multiple_per_device=True."""

    @pytest.mark.uses_gpu(1)
    @pytest.mark.slow
    def test_two_tools_share_one_gpu(self):
