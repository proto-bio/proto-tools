...
class TestMoveToDevice:
    ...
    def test_move_to_device(self, device_manager, mock_callback):
        ...
        ), "Should start on cuda:0"

        callback = MagicMock()
        device_manager.move_to_device("instance1", "cuda:1", worker_callback=callback)

        assert (
            device_manager._allocations["instance1"].device_ids[0] == "cuda:1"
        ), "Should move to cuda:1"
        callback.assert_called_once_with("cuda:1")

    def test_move_to_same_device(self, device_manager, mock_callback):
    ...
    def test_move_nonexistent_instance(self, device_manager, mock_callback):
    ...
    def test_move_to_generic_cuda(self, device_manager, mock_callback):
        """Test moving to generic 'cuda' resolves to specific device."""
        device_manager.request_device(
            "tool1", "instance1", device="cuda:0", eviction_callback=mock_callback
        )
        assert (
            device_manager._allocations["instance1"].device_ids[0] == "cuda:0"
        ), "Should start on cuda:0"
    ...
    def test_move_to_generic_cuda_triggers_lru_eviction(self, device_manager, mock_callback):
        ...
        assert (
            device_manager._allocations["instance2"].device_ids[0] == "cuda:0"
        ), "Instance2 should get cuda:0"
        assert (
            device_manager._allocations["instance1"].device_ids[0] == "cpu"
        ), "Instance1 should be on CPU"
        ...
        assert (
            device_manager._allocations["instance1"].device_ids[0] == "cuda:0"
        ), "Instance1 should move back to cuda:0"
    ...
    def test_move_to_generic_cuda_excludes_self_from_eviction(
        ...
        assert device_manager._allocations["instance1"].device_ids[0].startswith(
            "cuda"
        ), "Instance should stay on GPU"
...
class TestDeviceMismatchDetection:
    """Tests that _run_persistent detects when a tool's current device
    doesn't match the config's requested device and moves accordingly."""

    def _make_instance(self, device_manager, tool_name="mock_pytorch_tool", instance_name="inst"):
    ...
    def test_cpu_to_generic_cuda(self, device_manager, mock_callback):
        """Tool on CPU + config says 'cuda' → should move to GPU."""
        inst, mock_worker = self._make_instance(device_manager)
    ...
    def test_cpu_to_specific_cuda(self, device_manager, mock_callback):
    ...
    def test_wrong_gpu_to_specific_cuda(self, device_manager, mock_callback):
    ...
    def test_already_on_correct_gpu_no_move(self, device_manager, mock_callback):
    ...
    def test_generic_cuda_already_on_gpu_no_move(self, device_manager, mock_callback):
    ...
    def test_gpu_to_cpu(self, device_manager, mock_callback):
    ...
    def test_device_override_after_move(self, device_manager, mock_callback):
