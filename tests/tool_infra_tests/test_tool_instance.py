...
def _fake_instance_for_create_env(tmp_path: Path) -> ToolInstance:
    """Build a ToolInstance wired to a throwaway env dir, bypassing __init__."""
    inst = ToolInstance.__new__(ToolInstance)
    inst.toolkit = "fake_tool"
    inst.device = "cpu"
    inst.env_path = tmp_path / "fake_env"
...
def _tarball_clean_calls(mock_run) -> list:
    """Return the ``micromamba clean --tarballs`` invocations seen by a patched subprocess.run."""
    return [c for c in mock_run.call_args_list if "clean" in (c.args[0] if c.args else [])]


def _run_create_env(inst: ToolInstance, setup_returncode: int):
    """Drive _create_env with micromamba and setup.sh stubbed; return the subprocess.run mock."""

    def _make_env_dir(*args, **kwargs):
        inst.env_path.mkdir(parents=True, exist_ok=True)

    with (
        patch.object(inst, "_ensure_micromamba", return_value=Path("/fake/micromamba")),
        patch.object(inst, "_ensure_foundation_env", return_value=Path("/fake/foundation_env")),
        patch("proto_tools.utils.tool_instance.subprocess.run", side_effect=_make_env_dir) as mock_run,
        patch("proto_tools.utils.tool_instance._run_setup_script", return_value=(setup_returncode, "")),
    ):
...
def test_successful_build_cleans_package_archives(tmp_path: Path):
    """Archives are spent once extracted, so a successful build drops them."""
    inst = _fake_instance_for_create_env(tmp_path)
    mock_run = _run_create_env(inst, setup_returncode=0)

    calls = _tarball_clean_calls(mock_run)
    assert len(calls) == 1, "expected exactly one micromamba clean invocation"
    argv = calls[0].args[0]
    assert argv[1:] == ["clean", "--tarballs", "--yes"]
    # --packages would delete the extracted dirs micromamba hardlinks new envs from.
    assert "--packages" not in argv
    assert "--all" not in argv
    assert "MAMBA_ROOT_PREFIX" in calls[0].kwargs["env"]


def test_failed_build_keeps_package_archives(tmp_path: Path):
    """A failed build keeps its downloads so the retry does not re-fetch them."""
    inst = _fake_instance_for_create_env(tmp_path)
    mock_run = _run_create_env(inst, setup_returncode=42)
    assert _tarball_clean_calls(mock_run) == []


def test_archive_cleanup_is_best_effort(tmp_path: Path):
    """Cleanup must never turn a build that already succeeded into a failure."""
    with patch("proto_tools.utils.tool_instance.subprocess.run", side_effect=OSError("mamba gone")):
        ToolInstance._clean_package_archives(Path("/fake/micromamba"))
