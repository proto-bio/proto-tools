...
- **3D structure prediction:** ViennaRNA only predicts 2D base pairing, not 3D coordinates
- **Pseudoknots:** Standard MFE folding doesn't handle pseudoknots (use specialized tools)
- **Protein-RNA complexes:** RNA structure can change upon protein binding
...
- **ViennaRNA vs LinearFold:** LinearFold is faster (O(n) vs O(n³)) but slightly less accurate
...
|-----------|------|---------|-------------|-------------|
| `sequences` | `List[str]` | *required* | N/A | List of RNA/DNA sequences to fold |
| `temperature` | `float` | `37.0` | `25 - 42` | Temperature in Celsius; affects thermodynamic stability |
| `use_dna_params` | `bool` | `False` | N/A | Use DNA parameters instead of RNA |
| `no_lonely_pairs` | `bool` | `False` | N/A | Disallow isolated base pairs |

**Parameters to prioritize for sweeps:**
1. **`temperature`**: Most impactful for thermosensor designs or temperature-sensitive applications. Sweep 25°C, 30°C, 37°C, 42°C.
...
MFE interpretation depends heavily on sequence length. Normalized metrics:
...
**Parameter tuning:**
- **`temperature`**:
  - Use 37°C for physiological conditions
  - Lower temperatures (25°C) stabilize structures
  - Higher temperatures (42°C) destabilize; useful for thermosensor design
- **`use_dna_params`**:
...
3. **Forgetting T→U conversion:** Tool auto-converts T to U for RNA parameters
