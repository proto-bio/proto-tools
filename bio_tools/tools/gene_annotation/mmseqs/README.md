...
- Single Sequence Queries: If you're searching one sequence against NCBI, use `online-blast` instead.
...
- `online-blast` / `local-blast`: Use for smaller-scale searches or when you need NCBI database access.
- `hmmer`: Use for profile-based searches when detecting remote homologs (<30% identity).
- `diamond`: Another fast alternative to BLAST, particularly for translated searches (blastx/tblastn).
- `cd-hit`: Alternative clustering tool with different algorithmic approach.