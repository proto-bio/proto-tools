"""Tests for MAFFT tool in bio_programming.bio_tools.tools.sequence_alignment.mafft"""

import pytest

from bio_programming.bio_tools.tools.sequence_alignment.mafft import (
    MafftConfig,
    MafftInput,
    MafftOutput,
    run_mafft_align,
)
from bio_programming.bio_tools.tools.sequence_alignment.msas import MSA
from tests.tool_tests.tool_infra_tests.test_export_functionality import validate_output


class TestMafftOutput:
    """Tests for MafftOutput with embedded MSA."""

    @pytest.fixture
    def two_seq_output(self):
        return MafftOutput(
            msa=MSA(
    ...
    def test_basic_properties(self, two_seq_output):
    ...
    def test_to_fasta(self, two_seq_output):
        fasta = two_seq_output.msa.to_fasta_string()
        assert fasta == ">seq_0\nMVLS\n>seq_1\nAVLS"

    def test_msa_directly_accessible(self, two_seq_output):
...
class TestInputValidation:
    ...
    def test_valid_input_without_ids(self):
        inputs = MafftInput(sequences=["MVLSPADKTN", "MKLLVVAAAA"])
    ...
    def test_invalid_input(self, sequences, error_match):
        with pytest.raises(ValueError, match=error_match):
            MafftInput(sequences=sequences)


class TestConfigValidation:
    """Tests for MafftConfig validation."""
    ...
    def test_valid_align_methods(self, method):
        config = MafftConfig(align_method=method)
        assert config.align_method == method

    def test_invalid_align_method(self):
...
class TestMafftIntegration:
    """Integration tests requiring MAFFT installation.
    ...
    def test_protein_alignment_with_terminal_gaps(self):
    ...
    def test_protein_alignment_flanked_short_sequence(self):
    ...
    def test_dna_alignment_with_internal_gap(self):
    ...
    @pytest.mark.parametrize("method", ["auto", "localpair", "globalpair", "genafpair"])
    def test_all_alignment_methods_produce_correct_gaps(self, method):
    ...
    def test_config_options_passed_to_mafft(self):
    ...
    def test_to_fasta_output_format(self):
    ...
    def test_gap_statistics_accuracy(self):
    ...
    def test_sequence_ids_length_mismatch_fails(self):
