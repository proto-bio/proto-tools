...
- **Runtime:** Thorough sampling (25 samples × 200 steps) takes 2-10 minutes per complex
...
- **Hardware:** Requires GPU with ≥40GB VRAM (H100 or A100-80GB recommended)
- **Runtime:** ~2-10 minutes per complex depending on size and parameters on H100/A100 GPU
- **Network:** Requires internet access when using remote MSA search mode (ColabFold API)
...
|-----------|------|---------|-------------|-------------|
| `complexes` | `List[StructurePredictionComplex]` | *required* | N/A | List of complexes; each can have protein, DNA, RNA, and/or ligand chains |
...
| `recycling_steps` | `int` | `10` | `3 - 20` | Iterative refinement passes; more = better quality but slower |
| `sampling_steps` | `int` | `200` | `100 - 500` | Denoising steps; more = finer structures but slower |
| `diffusion_samples` | `int` | `25` | `5 - 50` | Independent structure samples; best is returned by confidence |
| `num_workers` | `int` | `min(cpu_count, 4)` | N/A | CPU workers for parallel processing |
| `devices` | `str` | `"0"` | N/A | GPU device IDs (e.g., "0" or "0,1" for multi-GPU) |
...
metrics = {
    ...
    "complex_pde": float,        # Predicted aligned error (Å), lower is better
    "complex_ipde": float,       # Interface PAE (Å), lower is better
}
```
...
| `complex_pde` | `float` | `>0 Å` | Spatial error estimate; <10Å = good relative positions, >20Å = uncertain |
...
- **Use high diffusion_samples (25+)** for protein-ligand and protein-nucleic acid complexes
...
- **Pre-filter sequences** by length to stay within GPU memory limits
