...
class BioToolsOnlyFilter(logging.Filter):
    """Filter to only allow logs from bio_programming_tools project packages."""
    def filter(self, record):
        # Include logs from bio_programming_tools and tests
        allowed_prefixes = ("bio_programming_tools", "tests")
...
def setup_logging(
    ...
    # Configure root logger for bio_programming_tools
    root_logger = logging.getLogger("bio_programming_tools")
    ...
    if log_to_file:
        if log_filename is None:
            ...
            log_file = log_path / f"bio_programming_tools_{timestamp}.log"
        else:
        ...
        # Add filter to only log bio_programming_tools messages to file
...
def get_logger(name: str) -> logging.Logger:
    ...
    """
    if not name.startswith("bio_programming_tools"):
        name = f"bio_programming_tools.{name}"
