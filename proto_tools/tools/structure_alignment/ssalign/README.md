...
- **Using it as a structural-novelty check.** `ss_score` is SSAlign's **predicted TM-score** (a linear fit of the embedding cosine), so higher means closer to a known fold and **low is novel**. Use it as a hard filter and **gate near your TM cutoff** (e.g. `ss_score < 0.4`), conservatively: it's a noisy, generically-calibrated estimate of the *average* (not minimum) chain TM, so keep margin and confirm borderline hits with `usalign`/`tmalign` for an exact TM. Search is **per chain** — a multi-chain query uses only its first chain, so score a complex one chain at a time.
