...
- **The backbone is loaded through Transformers, not the `esm` package.** The SAE API is defined on the Transformers ESM C model, so this toolkit loads `biohub/ESMC-300M` and siblings rather than the `esm`-package weights the `esmc` toolkit uses. The two therefore download separate copies of the backbone.
