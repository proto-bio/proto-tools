...
| `tools/tool_registry.py` (`cacheable`, `seed_sensitive`) | CLAUDE.md (Rules When Implementing Tools), `ToolRegistry` docstrings |
...
- **`@tool()`**: handles error catching, timing, metadata, registry, default config, device allocation validation, and cache policy. Set `seed_sensitive=True` on tools whose outputs depend on `config.seed`; cacheable unseeded calls skip cache/dedup so repeat dispatches advance RNG naturally. **Per-item seed handling is the tool's responsibility** — the framework does NOT unroll multi-item dispatches.
