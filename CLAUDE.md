...
| `tools/tool_registry.py` (`cacheable`, `stochastic`, `local_cpu`) | CLAUDE.md (Rules When Implementing Tools), `ToolRegistry` docstrings |
...
- `local_cpu` is **derived**, not declared. A tool is `local_cpu` iff `uses_gpu=False` AND no `standalone/` directory. `device='cloud'` is a no-op for `local_cpu` tools — they run in-process — and `ToolRegistry.list_local_cpu_tools()` is the contract the api inline router consumes. To opt a CPU tool *out* of `local_cpu` (because it's heavy, has a non-trivial env, or needs the cloud), give it a `standalone/` directory — the path `ccd_lookup`, `pyrosetta`, `blast`, `mmseqs2` take.
...
## Consuming proto-tools at runtime
...
proto-tools access <tool>                    # gated-weights access value
```
...
Before calling a tool that loads model weights, check how the weights are obtained:

```python
ToolRegistry.get_weights_access("esm3-embedding")   # "open" | "hf-gated" | "request"
```

`get_weights_access` normalizes the nested `license.yaml` `weights.access` field (absent => `"open"`) so there is one value to branch on. The same value is available raw as `get_license(...)["weights"]["access"]`, and via the CLI (`proto-tools access <tool>`). The values:

- `"hf-gated"`: weights are behind a gated HuggingFace repo. The user must accept the provider's terms and set `HF_TOKEN`, or the tool raises before loading. (e.g. ESM3, AlphaGenome)
- `"request"`: weights are not publicly distributed and must be obtained from the provider out of band. (e.g. AlphaFold3)

`commercial_use` (`"yes"` / `"no"` / `"restricted"`) and `attribution_required` (bool) carry the usage terms. The same restricted toolkits are also listed in the repo-root README's "Gated model access" table.
...
#### Claude Code dispatch flow

When a user asks Claude Code to run a bioinformatics tool:
...
| https://bio-pro.mintlify.app/tools | Rendered per-tool docs (auto-generated from READMEs and Pydantic field descriptions) |
